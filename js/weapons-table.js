...
async function displayTable() {
  try {
    const response = await fetch(`${url}weapons?order=weapon_id`);
...
addBtn.onclick = () => {
  window.location.assign(`weapons-edit.html`);
};
...
function onUpdateRow(e) {
  ...
  btn.onclick = () => {
    window.location.assign(`weapons-edit.html?weapon_id=${id}`);
  };
}
